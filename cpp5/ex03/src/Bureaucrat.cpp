...
// Methods
void Bureaucrat::signForm(AForm ref form) {
	if (form.getIsSigned())
	{
...
void Bureaucrat::executeForm(AForm cref form) const {
	if (!form.getIsSigned())
	{
		...
		PRINT BOLD AND form.getName() AND CLR PRP "\" because ";
		PRINT "it is not signed." AND std::endl;
		return ;
	}
	if (this->_grade > form.getExecGrade())
	{
		PRINT PRP "Bureaucrat " BOLD AND this->_name AND CLR PRP " couldn't execute form \"";
		PRINT BOLD AND form.getName() AND CLR PRP "\" because ";
		PRINT "he doesn't understand what he should do." AND std::endl;
		return ;
	}
	PRINT CYN "Bureaucrat " BOLD AND this->_name AND CLR CYN " executed form \"";
	PRINT BOLD AND form.getName() AND CLR CYN "\"." AND std::endl;
	form.execute(*this);
}
