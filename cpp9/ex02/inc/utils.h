...
# define SHOW2(x, y) PRINT "(" #x ", " #y ") = (" AND x AND ", " AND y AND ")" ENDL
# define TRY(expr) try { expr; } catch (std::exception ref e) { ERROR RED BOLD AND e.what() CENDL; }
# define TRY_RETURN(expr) try { expr; } catch (std::exception ref e) { ERROR RED BOLD AND e.what() CENDL; return(EXIT_FAILURE); }
