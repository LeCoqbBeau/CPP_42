...
# define TRY(expr) try { expr; } catch (std::exception ref e) { ERROR RED BOLD AND e.what() CENDL; }
# define TRY_RETURN(expr) try { expr; } catch (std::exception ref e) { ERROR RED BOLD AND e.what() CENDL; return (1); }
# define TRY_OPER(expr, oper)  try { expr; } catch (std::exception ref e) { ERROR RED BOLD AND e.what() CENDL; oper; }
